...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	alt_u32 base = 0x00000020;
	BYTE wdata[2] = {reg+2, val};
	if(alt_avalon_spi_command(base, 0, 2, wdata, 0, 0, 0) < 0){
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	if(alt_avalon_spi_command(base, 0, nbytes+1, wdata, 0, 0, 0) < 0){
...
BYTE MAXreg_rd(BYTE reg) {
	...
	BYTE wdata[1] = {reg};
	BYTE rdata[1];
	if(alt_avalon_spi_command(base, 0, 1, wdata, 1, rdata, 0) < 0){
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	BYTE wdata[1] = {reg};
	alt_u32 base = 0x00000020;
	if(alt_avalon_spi_command(base, 0, 1, wdata, nbytes, data, 0) < 0){
